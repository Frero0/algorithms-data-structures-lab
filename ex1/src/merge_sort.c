...
void merge(void *base, size_t l, size_t m, size_t r, size_t size, int (*compar)(const void *, const void *))
{
    ...
    memcpy(L, array + l * size, n1 * size);
    memcpy(R, array + (m + 1) * size, n2 * size);
    size_t i = 0, j = 0, k = l;
    while (i < n1 && j < n2)
    {
        if (compar((char *)L + i * size, (char *)R + j * size) <= 0)
        {
            memcpy(array + k * size, (char *)L + i * size, size);
            i++;
        }
        else
        {
            memcpy(array + k * size, (char *)R + j * size, size);
            j++;
        }
        k++;
    }
...
{
    if (base == NULL || compar == NULL || size == 0 || nitems < 2)
        return;
